...
namespace StokTakipSistemi
{
    public partial class Sayfalar : Form
    {
        ...
        // SifremiUnuttum.cs veya hangi form dosyanýzda çalýþýyorsanýz
        ...
        private void Sayfalar_Load(object sender, EventArgs e)
        {
            ...
            cmbBoxGenelK.Items.Add("Genel Kategori");
            cmbBoxUrunK.Items.Add("Ürün Kategorisi");
            cmbBoxFirmaAdi.Items.Add("Firma Adý");
            cmbBoxUrunTipi.Items.Add("Ürün Tipi");
            cmbBoxUrunModeli.Items.Add("Ürün Modeli");
            cmbBoxBoyut.Items.Add("Boyut");
            cmbBoxMensei.Items.Add("Mensei");

            //combobox'larýn bazýlarýnýn selected index'ini ayarlama
        ...
        private void btnUrunEkle_Click(object sender, EventArgs e)
        {
            ...
            string urunGirisTarihiText = mtxtBoxGTarihi.Text;   // textBox içindeki tarihi string bir deðiþkene atýyoruz aþaðýda çevirmek için
            DateTime urunGirisTarihiDate;  // string tarihi DateTime'a çevirmek için deðiþken

            if (urunGirisTarihiText.Contains("_"))
            {
                MessageBox.Show("Lütfen geçerli bir tarih giriniz.");
                return;
            }

            TextBox[] textBox = { cmbBoxAlisFiyati, cmbBoxSatisFiyati, txtBoxUBarkodu, txtBoxUAdi, txtBoxStok, txtBoxMarka };

            foreach (Control kontrol in textBox)
                if (string.IsNullOrWhiteSpace(kontrol.Text))
                {
                    MessageBox.Show($"{kontrol.Name} alaný boþ býrakýlamaz");
                    kontrol.Focus();
                    return;
                }

            ComboBox[] comboBox = { cmbBoxBoyut, cmbBoxGenelK, cmbBoxUrunK, cmbBoxFirmaAdi, cmbBoxUrunTipi, cmbBoxUrunModeli, cmbBoxMensei };

            foreach (Control kontrol in comboBox)
                if (string.IsNullOrWhiteSpace(kontrol.Text))
                {
                    MessageBox.Show($"{kontrol.Name} alaný boþ býrakýlamaz");
                    kontrol.Focus();
            ...
            if (string.IsNullOrWhiteSpace(cmbBoxBoyut.Text))
            {
                MessageBox.Show("Boyut alaný boþ býrakýlamaz");
                cmbBoxBoyut.Focus();
                return;
            }

            int stokMiktar = int.Parse(txtBoxStok.Text);
            float alisFiyati = float.Parse(cmbBoxAlisFiyati.Text);   // combombox'dan gelen alýþ fiyatýný float'a çeviriyoruz veritabaný için
            float satisFiyati = float.Parse(cmbBoxSatisFiyati.Text); // combombox'dan gelen satýl fiyatýný float'a çeviriyoruz veritabaný için
            decimal urunBoyut = int.Parse(cmbBoxBoyut.Text);   // combobox'dan gelen boyut deðerini decimal'e çeviriyoruz veritabaný için
            int urunBarkodu = int.Parse(txtBoxUBarkodu.Text);  // text'den gelen barkodu int deðere çeviriyoruz veritabanýnda int þeklinde

            /*Eðer giriþ tarihi yanlýþ deðilse ve DateTime çevirme iþlemi gerçekleþtiyse ve urunKategori boþ býrakýlmadýysa
             urunIslemleri class'ýnýn urunEkle fonksiyonunu çaðýrýp veritbanýna ürünü ekliyoruz, eðer ekleme iþlemi sorunsuz geçmiþ olursa
             Sayfalar class'ýnýn urunEkleArayuzGuncelle fonksiyonunu çaðýrýp combobox'larýn içine de bu verileri giriyoruz.*/
            if (DateTime.TryParse(urunGirisTarihiText, out urunGirisTarihiDate) && sqlIslemleri.urunVarMi(urunBarkodu))
                if (urunIslemleri.urunEkle(urunBarkodu, txtBoxUAdi.Text, cmbBoxUrunK.Text, cmbBoxFirmaAdi.Text, cmbBoxUrunTipi.Text,
                cmbBoxUrunModeli.Text, urunBoyut, cmbBoxMensei.Text, alisFiyati, satisFiyati, urunGirisTarihiDate,
                txtBoxMarka.Text, stokMiktar))
                    urunEkleArayuzGuncelle(cmbBoxGenelK.Text, cmbBoxUrunK.Text, cmbBoxFirmaAdi.Text, cmbBoxUrunTipi.Text,
                                            cmbBoxUrunModeli.Text, cmbBoxBoyut.Text, cmbBoxMensei.Text);
        }
        ...
        // silinecek mi bakarsýnýz hata veriyor cmbox ý sildim çünkü
        /*
                private void cmbBoxStokSFiltre_TextChanged(object sender, EventArgs e)
                {
                    SQLIslemleri sQLIslemleri = new SQLIslemleri();

                    sqlIslemleri.urunleriCek(cmbBoxStokSFiltre.Text, dataGViewStok);
                }
        */
        ...
        {
            TabPage istatistikPage = tabControl1.TabPages["tabPageIstatistik"]; // tabIstatistik yerine sizin tab page isminizi yazýn

            if (this.WindowState == FormWindowState.Maximized)
            {
                // Tam ekran durumunda panelleri daha geniþ yap
                panelUrunBazindaSatis.Width = (int)(istatistikPage.Width * 0.9);
                panelUrunKarOraný.Width = (int)(istatistikPage.Width * 0.9);
            }
            else
            {
                panelUrunBazindaSatis.Width = istatistikPage.Width / 2;
                panelUrunKarOraný.Width = istatistikPage.Width / 2;
            }
        }
    }
}
